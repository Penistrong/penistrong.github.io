...
在put流程的`putVal()`函数返回前，会调用`addCount()`方法，增加当前CHM里的元素总数，并判断当前哈希表是否需要被扩容？如果需要扩容，且当前并不在扩容，则初始化`transfer()`方法；如果需要扩容且当前有其他线程正在扩容，则本线程一起帮助扩容，执行`helpTransfer()`方法
...
上述代码中第3行使用Unsafe类提供的`compareAndSetLong`这个CAS更新Long类型值的函数，`BASECOUNT`为字段`baseCount`在CHM类中的偏移量，CAS函数根据当前CHM对象的内存起始地址加上该偏移量，取出`baseCount`的实际值与当前线程可见的`baseCount`预期值进行比较，相符则说明没有其他线程在竞争，于是更新为新的元素个数
...
`stride`为每个物理机上CPU处理的桶数量，用当前哈希表大小除以8(右移3位)再除以CPU的个数，当然`stride`最小为`MIN_TRANSFER_STRIDE=16`

扩容时，是将旧的哈希表`tab`中的桶迁移到新的哈希表`nextTab`中
