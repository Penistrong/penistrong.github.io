...
header-img: "img/post-bg/2023-01-17-[Java] JVM内存模型与垃圾回收.jpg"
...
HotSpot JVM为老年代划分了一大块称为Tenured Space的区域。新生对象首先在Eden区域分配内存，执行一次新生代区域的垃圾回收后(至少为Minor GC)，如果对象仍然存活则最终会进入From区域中(关于GC过程具体见[垃圾回收](#垃圾回收-gc)一节)，对象的年龄计数器也会+1，当年龄满足晋升阈值后就会被移入Tenured里
...
.class字节码文件剩余的部分，比如方法与字段数据、方法与构造函数的字节码等，仍被存储到元空间里，但不是运行时常量池的组成部分
...
注意，`StringTable`保存的是对字符串对象的**引用**，实际仍指向堆中xx代区域里实际存在的字符串对象
