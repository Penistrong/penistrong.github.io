...
对于给定的相机外部参数 ${\xi}$，令 $\{\mathbf{x}_{j}\}_{j=1}^{N_s}$ 为给定像素点沿相机射出光线 $\mathbf{d}$ 的各个不同采样点($N_s$为采样点个数)，并且令 $(\sigma_{j}, \mathbf{f}_j)=C(\mathbf{x}_j, \mathbf{d})$ 为采样点 $\mathbf{x}_{j}$ 的特征场对应的体积密度和特征向量。体绘制算子 $\pi_{\textrm{vol}}$ 将这些采样点的评估映射到该像素点的最终特征向量 $\mathbf{f}$:
