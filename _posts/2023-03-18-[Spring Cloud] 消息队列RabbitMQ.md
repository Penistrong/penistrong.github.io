...
- 生产者到RabbitMQ：生产者发送消息给RabbitMQ后，需要知道自己是否成功发送了消息，一般称为 消息发送确认，RabbitMQ提供事务机制或者Confirm机制以实现生产者确认机制
  ...
  2. **Confirm机制**：通过`confirmSelect()`将信道设置为Confirm模式，所有在该信道上发布的消息都会被分配1个唯一ID，当生产者发送消息(`Basic.Publish`消息，携带消息头和消息体)给RabbitMQ后，如果RabbitMQ成功通过交换机将这条消息路由到目的队列中，它就会发布一条`Basic.Ack`确认消息(携带消息的唯一ID)告知生产者。生产者如果发现超时未收到`Basic.Ack`，就会重传该消息；如果收到`Basic.NAck`，生产者就可以做对应的处理
...
- RabbitMQ本身：
  ...
  3. RabbitMQ到消费者:
     ...
     - **消息应答机制**：
       ...
       当消费者因为宕机一直没有发送ack应答消息，那么RabbitMQ就认为该消息没有被处理，将其重新入队，分发给其他消费者，保证消息不会丢失
